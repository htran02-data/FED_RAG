...
DB_PATH = pathlib.Path(__file__).resolve().parent / "fed.db"
VECTORS_PATH = pathlib.Path(__file__).resolve().parent / "vectors.npy"
...
CREATE TABLE IF NOT EXISTS chunks (
    ...
    source_url   TEXT,
    word_count   INTEGER,
    -- Position of the passage inside its section, so retrieval can widen a hit
    -- to its neighbours at read time instead of embedding overlapping copies.
    para_index   INTEGER,
...
# Columns added after the first stores were built. Each is metadata only, so an
# existing store can gain the feature without being re-embedded.
MIGRATIONS = [
    ("para_index", "ALTER TABLE chunks ADD COLUMN para_index INTEGER"),
]
...
def connect(db_path=DB_PATH):
    ...
    with conn:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_chunks_neighbour "
                     "ON chunks(meeting_date, doc_type, section, para_index)")
    return conn
...
def _write_batch(batch, vectors, conn, vectors_path):
    ...
    rows = [
        (c["content_hash"], c["chunk_id"], c["meeting_date"], c["year"],
         ...
         c.get("source_url", ""), c.get("word_count", 0),
         (c.get("paragraphs") or [None])[0], offset + i)
...
def backfill_para_index(conn, chunks):
    """
    Add paragraph positions to a store that predates the column.

    This is metadata only -- no vector changes, so an existing store gains
    read-time expansion without being re-embedded.
    """
    updates = [((c.get("paragraphs") or [None])[0], c["content_hash"]) for c in chunks]
    with conn:
        conn.executemany(
            "UPDATE chunks SET para_index = ? WHERE content_hash = ?", updates)
    return sum(1 for u in updates if u[0] is not None)
...
def main():
    ...
    # Separate paths let two chunking schemes be embedded side by side and
    # compared, instead of one silently accumulating into the other's store.
    ...
    vectors_path = pathlib.Path(args.vectors)
    ...
    count, vectors, problems = verify(conn, vectors_path)
