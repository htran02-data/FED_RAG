...
No LangChain, no LlamaIndex, no hosted vector DB. The corpus is ~11,000 chunks;
...
ask.py       retrieval primitives and compatible CLI entry point
fedrag/      shared service, configuration, terminal interface
fed          executable terminal launcher
app.py       Streamlit UI
...
- **Embed one paragraph per chunk, then widen the winners at read time.** A hit
  is expanded to its neighbours inside the same section only after it has been
  scored (`ask.expand_hits`, on by default). Embedding overlapping copies
  instead was measured and rejected: it costs 30% more tokens, makes every
  vector describe three paragraphs at once, and reaches the same recall that
  read-time widening reaches for nothing. Expansion must never cross a section
  boundary, for the attribution reason below.

Measured on the 30-pair gold set: widening takes quote recall from 0.900 to
1.000 and fixes all three answers that straddle a paragraph break. Reranking
(`--rerank`) adds 0.036 MRR on top but cannot improve recall, and costs an API
call per question, so it stays opt-in.
...
- The gold set must keep testing the hard cases, not just the easy ones. It
  carries answers that span a paragraph break and questions that state no
  period; each pair records `spans_paragraphs` and `has_temporal_cue`, and the
  tests assert both against the corpus and the parser, so a pair cannot quietly
  stop testing what it exists for. A gold set of only single-paragraph,
  date-cued questions scores 1.000 on everything and measures nothing.
- Score retrieval on the answer quote, not on `content_hash`. Re-slicing the
  corpus changes every hash, so a hash-keyed gold set silently drops to zero
  the moment chunking changes and can never compare two schemes.
