...
GOLD_PATH = pathlib.Path(__file__).resolve().parent / "data/gold.jsonl"


QUERY_CACHE = pathlib.Path(__file__).resolve().parent / "data/query_vectors.npz"
...
def quote_matches(text, answer_key):
    """
    True when a chunk contains the gold passage's words.

    A gold answer_key may elide with "..." to skip an irrelevant clause, so
    every fragment must appear, in order. Matching on the quote rather than on
    a content_hash is what lets two different chunking schemes be compared: the
    hash changes when the slicing changes, the words do not.
    """
    haystack = _flatten(text)
    cursor = 0
    for fragment in (f.strip() for f in _flatten(answer_key).split("...")):
        if not fragment:
            continue
...
def quote_rank(hits, item):
    """
    1-based rank of the first hit that carries the gold words *and* comes from
    the gold meeting. The meeting check matters because the Fed reuses phrasing
    across years, so the same sentence genuinely occurs in several meetings.
    """
    for position, hit in enumerate(hits, start=1):
        if (hit["meeting_date"] == item["meeting_date"]
                and quote_matches(hit["text"], item["answer_key"])):
            return position
    return None
...
def evaluate(gold, conn, matrix, k=8, use_filter=True, era_names=None,
             embedder=None, verbose=False, hybrid=True, expand=0,
             reranker=None):
    ...
    for item in gold:
        ...
        qrank = quote_rank(hits, item)
        words_shown = sum(len(h["text"].split()) for h in hits)
...
def summarize(results, k):
    ...
    return {
        "n": total,
        # Quote metrics are the ones that survive a change of chunking: they
        # ask whether the gold words reached the reader, not whether a
        # particular slice id did.
        f"quote_recall@{k}": sum(1 for r in results if r["quote_rank"]) / total,
        "quote_mrr": sum(1 / r["quote_rank"] for r in results if r["quote_rank"]) / total,
        ...
        "mean_words_shown": sum(r["words_shown"] for r in results) / total,
...
def print_comparison(before, after, k):
    ...
    print("  " + "-" * 66)
    for key in (f"quote_recall@{k}", "quote_mrr", f"recall@{k}",
                f"meeting_recall@{k}", "mrr", "wrong_meeting_rate",
                "mean_words_shown", "mean_candidates"):
        low, high = before[key], after[key]
        if key in ("mean_candidates", "mean_words_shown"):
...
def main():
    ...
    parser.add_argument("--db", help="store to evaluate (default fed.db)")
    parser.add_argument("--vectors", help="matrix to evaluate (default vectors.npy)")
    ...
    gold = load_gold()
    conn, matrix = ask.load_store(args.db, args.vectors)
